angular.module("myApp")
    .controller('ArticleList', ['$scope','$http','type','status','$state','$stateParams','$filter',function ($scope, $http,type,status,$state,$stateParams,$filter) {
        //初始显示页面
        //刷新后取出参数给现有的数据赋值
        var arti=this
    ...
    //清除点击事件
        arti.cleanSeach = function (clean) {
            $state.go('carrots.Artical-list',
                {
                    type: undefined,
                    status:undefined,
            ...
            bootbox.confirm({
                ...
                callback:function (result) {
                    if(result===true){
                        $http({
                            method:"DELETE",
                            url: '/carrots-admin-ajax/a/u/article/'+arti.getId,
                            headers: {'Content-type': 'application/json'}
                        }).then(function (response) {
                            if(response.status===200&&response.data.code===0) {
                                $state.go('carrots.Artical-list',
